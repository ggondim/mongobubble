...
import { MongoRepository, ObjectIdEntity } from '@mongobubble/core';
import { JsonSchemaValidationPlugin } from '@mongobubble/json-schema/';
import TsEdPlugin from '../src/TsEdPlugin';
export class UserWithoutDecorators extends ObjectIdEntity<UserOid> {
...
function newUserRepository() {
  return new MongoRepository<UserOid>(UserOid, {
    db,
    plugins: [JsonSchemaValidationPlugin, TsEdPlugin]
  });
}
...
describe('TS.ED', () => {
  ...
  test('no schema generation', async () => {
    const schema = getJsonSchema(UserWithoutDecorators);
    console.log(schema)
  });
